...
function [hIm] = ScSR(lIm, Dh, Dl, sparse)
...
mIm = double(imresize(lIm, sparse.up_scale, 'bicubic'));
...
gridx = 3:patch_size - sparse.overlap : w-patch_size-2;
gridx = [gridx, w-patch_size-2];
gridy = 3:patch_size - sparse.overlap : h-patch_size-2;
...
for ii = 1:length(gridx),
    for jj = 1:length(gridy),
        ...
        % sparse recovery
        w = L1QP_FeatureSign_yang(sparse.lambda, A, b);
