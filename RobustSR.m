...
function HR=RobustSR(LR, HR, im_h, props)
% im_h: the result of sparse representation
% HR: the initial HR image
% LR: the LR images
...
while iter<props.maxIter
  ...
  Gback = GradientBackProject(HR, LR, props.T);
  
  % Compute the gradient of the bilateral filter part of the cost function
  Greg = GradientRegulization(HR, im_h, props.P);
